---
layout: post
title: "웹 폰트와 아이콘 적용법 — Google Fonts, Font Awesome, SVG 아이콘"
description: "웹 폰트와 아이콘은 어떻게 적용하나요? Google Fonts CDN 연결, @font-face로 직접 호스팅, font-display 최적화, Font Awesome 아이콘 사용, SVG 스프라이트 아이콘 시스템까지 다룹니다."
...
기본 시스템 폰트만 쓰면 디자인에 한계가 있습니다. 웹 폰트를 적용하면 어떤 기기에서도 동일한 폰트로 보이는 일관된 디자인을 만들 수 있습니다. Google Fonts 적용부터 성능 최적화까지 알아봅니다.
...
벡터 아이콘을 웹에서 편하게 쓸 수 있는 라이브러리입니다.
...
## 직접 해보기: font-display 차이 비교

`font-display: swap`과 `font-display: block`의 차이는 네트워크 탭에서 Slow 3G로 속도를 제한하면 확실히 체감됩니다. `swap`은 텍스트가 바로 보이고 나중에 폰트가 바뀌고, `block`은 폰트가 다운로드될 때까지 텍스트가 보이지 않습니다.

SVG 아이콘과 Font Awesome 아이콘의 차이는 성능에서 나타납니다. Font Awesome은 CSS 파일 + 폰트 파일을 모두 로드해야 하지만, 인라인 SVG는 추가 HTTP 요청이 없습니다. 블로그처럼 아이콘이 몇 개만 필요할 때는 SVG가 훨씬 가볍습니다.
...
다음 글에서는 Open Graph 태그 심화 — SNS 공유 최적화를 알아봅니다.