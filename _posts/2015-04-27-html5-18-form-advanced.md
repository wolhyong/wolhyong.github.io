---
layout: post
title: "시맨틱 HTML 심화 — aside, figure, time, address, details 활용법"
description: "시맨틱 HTML 태그에는 어떤 것들이 더 있을까요? aside, figure, figcaption, time, address, details, summary, mark, abbr 태그의 올바른 사용법과 SEO·접근성 향상 효과를 예제와 함께 설명합니다."
...
<main>
  ...
  <aside>
    <h2>관련 글</h2>
    <ul>
      <li><a href="/post/pip-guide">pip 패키지 관리</a></li>
...
FAQ 섹션, 설치 방법을 단계별로 설명하거나, 코드 설명 등에 활용합니다.

---

## abbr — 약어 설명

`<abbr>`는 약어에 전체 의미를 부여합니다. 마우스를 올리면 툴팁으로 표시됩니다. 스크린 리더도 이 정보를 읽어주기 때문에 접근성 측면에서도 좋습니다.

직접 브라우저에서 `<abbr title="HyperText Markup Language">HTML</abbr>` 위에 마우스를 올려보면 툴팁이 나타납니다.
...
<!-- 인용문에서 중요 부분 강조 -->
<blockquote>
  <p>코드는 작동하게 만드는 것보다 <mark>읽히게 만드는 것</mark>이 더 필요합니다.</p>
</blockquote>
```
...
git push origin main</code></pre>
    ...
    <aside aria-label="관련 글">
      <h2>관련 글</h2>
      <ul>
        <li><a href="/post/git-branch">Git 브랜치 전략 알아보기</a></li>
...
1. `<details>`와 `<summary>`를 이용한 FAQ 섹션을 만들고 브라우저에서 펼치고 접어보기
2. `<figure>`와 `<figcaption>`으로 이미지+설명 묶음을 만든 후 화면에서 어떻게 표시되는지 확인
3. `<abbr title="">`에 마우스를 올리면 툴팁이 나타나는지 직접 테스트
