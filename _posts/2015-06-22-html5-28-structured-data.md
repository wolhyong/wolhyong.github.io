...
description: "웹 접근성(a11y)과 ARIA 속성은 어떻게 적용하나요? ARIA role, aria-label, aria-hidden, aria-expanded, aria-live 속성의 올바른 사용법, 키보드 탐색 지원, 색상 대비 체크리스트까지 바로 적용할 수 있는 내용을 소개합니다."
...
| **R**obust (견고함) | 여러 보조 기술에서 동작해야 함 |

---

## 시맨틱 HTML이 최선의 접근성

ARIA를 쓰기 전에, 올바른 시맨틱 태그를 먼저 사용하는 것이 훨씬 필요합니다.
...
<article>
  <h2>React 입문</h2>
  <a href="/post/2" aria-label="React 입문 더 읽기">더 읽기</a>
</article>
```
...
[WebAIM Contrast Checker](https://webaim.org/resources/contrastchecker/)에서 색상 대비를 확인할 수 있습니다.
...
처음부터 모든 것을 완벽하게 하려고 하면 부담됩니다. 이 중에서 가장 효과가 큰 것부터 적용해보세요.

1. 모든 이미지에 `alt` 속성 붙이기 (장식용 이미지는 `alt=""`)
2. 모든 form input에 `<label>` 연결하기
3. 키보드만으로 모든 버튼과 링크를 누를 수 있는지 확인하기
4. `<html lang="ko">` 확인하기
---
...
다음 글에서는 웹 폰트와 아이콘 — Google Fonts, Font Awesome 적용 방법을 알아봅니다.