---
layout: post
title: "HTML5 SVG 기초 — 인라인 SVG로 벡터 그래픽 만들기"
description: "HTML5 SVG는 Canvas와 무엇이 다를까요? HTML에 SVG를 삽입하는 방법과 rect, circle, line, path, polygon 기본 도형 요소, viewBox 개념, CSS로 SVG 스타일링, 아이콘 제작 예제까지 다룹니다."
...
브라우저에서 SVG로 그린 원과 Canvas로 그린 원을 각각 `Ctrl++`로 확대해보세요. SVG는 끝없이 확대해도 가장자리가 깨끗한 반면, Canvas는 픽셀이 보이기 시작합니다. 이 차이가 벡터(SVG)와 래스터(Canvas)의 핵심 차이입니다.

또한 개발자 도구에서 SVG 요소를 선택해보세요. circle, rect 같은 각 도형이 DOM 요소로 보입니다. 즉 CSS로 직접 스타일을 바꾸거나 JavaScript로 이벤트를 걸 수 있습니다. Canvas에서는 이렇게 할 수 없습니다.

---

## 핵심

- SVG는 벡터 그래픽으로 확대해도 깨지지 않습니다 (로고, 아이콘에 적합)
- `viewBox`로 내부 좌표계를 독립적으로 관리합니다
- `rect`, `circle`, `line`, `path` 등 기본 도형 요소를 조합합니다
- 인라인 SVG는 CSS와 JavaScript로 직접 제어 가능합니다 (DOM 요소)
- `currentColor`로 CSS 색상과 SVG 색상을 연동합니다

다음 글에서는 picture 태그와 반응형 이미지 — srcset, sizes를 알아봅니다.