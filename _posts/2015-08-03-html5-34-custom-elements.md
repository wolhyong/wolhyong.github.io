---
layout: post
title: "반응형 이미지 — picture, srcset, sizes 속성 사용법"
description: "반응형 이미지는 어떻게 최적화하나요? picture 태그와 img의 srcset, sizes 속성으로 화면 크기와 해상도에 따라 최적 이미지를 제공하는 방법, 1x/2x 디스플레이 대응, WebP 포맷 폴백 패턴, Core Web Vitals 개선까지 다룹니다."
...
단순히 해상도가 다른 것이 아니라, 화면 크기에 따라 **전혀 다른 이미지**를 보여줄 때 사용합니다.
...
직접 실습하려면 뷰포트를 400px로 줄이고 새로고침한 후, 네트워크 탭에서 이미지 탭을 확인해보세요. 1200w 이미지 대신 400w 이미지가 다운로드된 것을 확인할 수 있습니다. 반대로 뷰포트를 1400px로 늘리면 1200w 이미지가 다운로드됩니다. 브라우저가 `sizes` 조건에 따라 알아서 선택합니다.

`loading="lazy"`가 적용된 이미지는 네트워크 탭에서 스크롤하기 전에는 다운로드되지 않는 것을 확인할 수 있습니다. LCP 이미지(첫 화면에 보이는 이미지)에는 `loading="eager"`를 주는 것이 좋습니다.
...
다음 글에서는 HTML template 태그와 동적 콘텐츠 생성을 알아봅니다.